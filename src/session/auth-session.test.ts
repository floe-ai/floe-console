...
import type { AuthenticateResult, ChallengeGrant } from "../bus/identity-auth.js";
 ...
 * membership -> ready, several -> selection required, 401 -> admission wait,
 * 403 -> wrong workspace. This is client logic under test, not a substrate mock:
 * no substrate state is fabricated, only the documented transport responses.
 */
...
function makeSession(authenticate: (workspaceId?: string) => Promise<AuthenticateResult>) {
...
describe("AuthSession", () => {
  ...
  it("waits for admission (human Check now) on a 401 identity_auth_failed", async () => {
    let attempts = 0;
    const { session } = makeSession(async () => {
      attempts += 1;
      if (attempts === 1) {
    ...
    const waiting = await session.unlock("pw");
    expect(waiting.kind).toBe("awaiting-admission");

    // The human admits the key out of band, then triggers a re-check.
